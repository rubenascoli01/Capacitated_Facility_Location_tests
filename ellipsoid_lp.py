...
def ellipsoid_oracle(
    ...
    *,
    max_iter: int = 10_000,
    tol: float = 1e-2,
    R: float = 1e1,# final polytope fits in a hypercube in all cases
    x0: np.ndarray = None,
    extras = None,
    stop_early=False,
    find_feas=False,
    ...
    def ellipsoid_update(
        ...
        if gPg <= 0.0:
            ...
            exit()
            return x, P          # degenerate / zero subgradient — skip
    ...
    for k in range(max_iter):
        ...
        if not feasible:
            ...
            # print("Feasibility confirmed: improving objective...")
            # ---- Phase II: x is feasible — try to improve objective -----
            obj_val = float(c @ x)
            if printer: print(f"found feasible point at iteration {k+1} with objective {obj_val}")
            if obj_val < best_obj:
                best_obj = obj_val
                best_x = x.copy()
