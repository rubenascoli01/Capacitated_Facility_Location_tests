...
def CFL_oracle(var, prob, debug=False): # var is y + flattened x in one
    ...
    #     if var[i] > 1:
    #         # print("violated box <= 1 constraint")
    #         return False, (np.eye(len(var))[i], 1.0)
    #     elif var[i] < 0:
    #         # print("violated box >= 0 constraint")
    #         return False, (-1 * np.eye(len(var))[i], 0.0)
    ...
    I = list(np.where(y_prime == 1)[0])
    S = list(np.where(y_prime != 1)[0])
    ...
    if num_I > 0:
        ...
        z = m.addMVar((num_I, num_D), vtype=gp.GRB.CONTINUOUS, lb=0.0, ub=1.0, name="z")

        for i in range(num_I):
            idx = I[i]
            m.addConstr(z[i, :].sum() <= U[idx], name=f"B_facilities{i}")
        ...
        # Step 4
        rows1, cols1 = np.where(z_vals < 2 * x[I, :])
        ...
        H = nx.Graph()
        H.add_nodes_from([j for j in range(num_D)] + [num_D + i for i in range(num_I)])
        ...
        if zero_test:
            # Test with 0
            d = [1 for j in range(prob.num_D)] # demands
            feas, (arr1, arr2) = MFN_feas_test(g, x, y, prob, S, d)

            if not feas:
                # print("violated zero constraint")
                return feas, (arr1, arr2) # if infeasible, return the violated constraint
            else:
                pass # do not return g as the feasible point, continue to the g we constructed

        # Test with constructed g
