...
epsilon = 0.01 # Algorithm of Pat et al. guarantees a (9+epsilon)-approximation
...
def LocalSearch(prob, printer=True, writer=False, verbose=False):
	...
	step = 0
	print("Starting S: "+str(S)+"; starting val: "+str(val))
	while True: #check to see whether any of the local search operations can lower the objective value
		...
		if bestNewVal <= val*(1-epsilon/(prob.num_F)):
			S = bestNewS
			val = bestNewVal
			print("Step = "+str(step)+"; chosen operation: "+best_op+"; new S = "+str(S)+"; new cost = "+str(val))
		else: # If none of the operations yields a small improvement, we're done.
			print("Step = "+str(step)+"; no operation yielded enough improvement.")
			break
	return val
